...
public class JpaKataApplication {
    ...
    InitializingBean populateData(final CustomerService customerService) {
        return () -> {
            ...
            customerService.persistCustomer(TCustomer.builder().login("ironMike").name("tyson").salary(new BigDecimal(42)).build());
            customerService.persistCustomer(TCustomer.builder().login("royjones").name("jones").salary(new BigDecimal(52)).build());
            customerService.persistCustomer(TCustomer.builder().login("money").name("mayweather").salary(new BigDecimal(27642)).build());
            customerService.persistCustomer(TCustomer.builder().login("manny").name("pacquiao").salary(new BigDecimal(8244)).build());
            ...
            TCustomer trash = customerService.persistCustomer(TCustomer.builder().login("trash").name("trash").salary(new BigDecimal(1)).build());
